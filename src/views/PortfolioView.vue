...
import PortfolioGlow from '../components/ui/PortfolioGlow.vue'
import PortfolioNetworkArt from '../components/ui/PortfolioNetworkArt.vue'
...
<template>
  <main ref="portfolioRoot" class="portfolio-page w-full pb-20 no-print">
    <section class="group/section reveal relative isolate min-h-[calc(100vh-3.5rem)] overflow-hidden border-b border-gray-100 px-[clamp(1.25rem,7vw,8.5rem)] py-20 dark:border-gray-800 sm:py-28">
      <PortfolioNetworkArt gradient-id="portfolio-hero-gradient" variant="orbit" class="-left-72 right-auto scale-[1.6] -rotate-12 opacity-100" />
      <div class="portfolio-hero-copy relative z-10 flex min-h-[calc(100vh-9rem)] max-w-5xl flex-col justify-center">
        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-blue-600 dark:text-blue-400 sm:text-base">
          {{ lang === 'zh' ? '網頁設計與全端開發' : 'Web design & full-stack development' }}
        </p>
        <h1 class="mt-5 max-w-5xl text-5xl font-bold tracking-tight text-gray-950 dark:text-white sm:text-8xl">
          {{ lang === 'zh' ? '把想法做成帶來結果的網站。' : 'Build a website that moves your business forward.' }}
        </h1>
        <p class="mt-6 max-w-3xl text-lg leading-8 text-gray-600 dark:text-gray-300 sm:text-xl sm:leading-9">
        ...
        <div class="mt-8 flex flex-wrap gap-3">
          <button type="button" class="rounded-lg bg-blue-600 px-6 py-4 text-base font-semibold text-white transition hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2 dark:focus:ring-offset-gray-950" @click="scrollTo('contact')">
            {{ lang === 'zh' ? '討論你的專案' : 'Start a conversation' }}
          </button>
          <button type="button" class="rounded-lg border border-gray-300 px-6 py-4 text-base font-semibold text-gray-700 transition hover:border-blue-500 hover:text-blue-600 focus:outline-none focus:ring-2 focus:ring-blue-500 dark:border-gray-700 dark:text-gray-200" @click="scrollTo('projects')">
            {{ lang === 'zh' ? '查看作品' : 'View selected work' }}
          </button>
        </div>
      </div>
      <div class="pointer-events-none absolute -right-20 -top-24 h-[30rem] w-[30rem] rounded-full bg-blue-500/15 blur-3xl" aria-hidden="true" />
    </section>

    <section id="services" class="group/section reveal relative isolate scroll-mt-20 overflow-hidden border-b border-gray-100 px-[clamp(1.25rem,7vw,8.5rem)] py-16 dark:border-gray-800 sm:py-20">
      <PortfolioNetworkArt gradient-id="portfolio-services-gradient" variant="grid" class="-left-56 right-auto scale-90 rotate-180 opacity-80" />
      <PortfolioGlow class="right-[-10rem] top-[-8rem] h-72 w-72 bg-indigo-500/15" />
    ...
    <section id="results" class="group/section reveal relative isolate scroll-mt-20 overflow-hidden border-b border-gray-100 px-[clamp(1.25rem,7vw,8.5rem)] py-16 dark:border-gray-800 sm:py-20">
      <PortfolioNetworkArt gradient-id="portfolio-results-gradient" />
      <div class="relative grid gap-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-end">
        <div>
    ...
    <section id="process" class="group/section reveal relative isolate scroll-mt-20 overflow-hidden border-t border-gray-100 px-[clamp(1.25rem,7vw,8.5rem)] py-16 dark:border-gray-800 sm:py-20">
      <PortfolioNetworkArt gradient-id="portfolio-process-gradient" variant="rings" class="-right-72 scale-90 opacity-60" />
      <PortfolioGlow class="left-[-12rem] top-1/3 h-80 w-80 bg-violet-500/15" />
      <div class="max-w-3xl">
    ...
    <section id="contact" class="group/section reveal relative isolate scroll-mt-20 overflow-hidden border-t border-gray-100 px-[clamp(1.25rem,7vw,8.5rem)] py-16 dark:border-gray-800 sm:py-20">
      <PortfolioNetworkArt gradient-id="portfolio-contact-gradient" variant="waves" class="-left-64 right-auto scale-90 rotate-180 opacity-50" />
      <PortfolioGlow class="right-[-8rem] bottom-[-10rem] h-96 w-96 bg-cyan-500/15" />
