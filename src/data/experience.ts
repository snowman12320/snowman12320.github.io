...
export const experiences: ExperienceItem[] = [
  {
    ...
    contribution: {
      zh: '[{"title":"性能提升","items":["支援 5,000+ 並發 WebSocket 連線，實時直播聊天與互動不延遲（透過 Socket.IO 房間分片與 Redis 轉發）","頁面加載時間從 3.2s 優化至 0.8s（75% 性能提升）（透過路由懶加載、Tree Shaking 與資源預載）","導播控制台響應延遲控制在 < 100ms，提升操作體驗（透過虛擬滾動與 WebSocket 狀態同步）","無限滾動媒體資源管理，支援 10,000+ 媒體項目無感加載（透過 Intersection Observer 與分頁預取）"]},{"title":"程式品質","items":["開發 32 個可復用 Vue 3 Composables 元件，提升代碼復用率 68%（抽象共用邏輯並統一錯誤處理）","建置 45 個模組化元件，支援直播頁、導播控制、會員中心等功能（採用 Atomic Design 與 props 驅動）","單元測試覆蓋率達 82%，確保代碼穩定性（以 Vitest + Vue Test Utils 覆蓋核心流程）","搭配 12 個 MSW API Mock 方案，加快前後端並行開發（定義標準 API schema 供前後端共用）"]},{"title":"商業影響","items":["整合 Payuni 金流、Keycloak 單一登入與 RBAC 角色權限（封裝統一認證與授權中介層）","實作 4 種即時通知機制（WebSocket、SSE、訂閱推播、期限提醒）（採用事件驅動架構與通知排程器）","支援聯合直播、共同直播、OBS 導播等 3 種直播模式（模組化串流控制與場景切換邏輯）","會員中心流程優化，訂閱轉換率提升 35%（簡化結帳步驟並導入 A/B 測試驗證）"]},{"title":"開發效率","items":["Jenkins CI/CD 自動化部署，每週 8-12 次自動部署（透過 Pipeline as Code 與自動化測試閘道）","Git 分支管理與代碼審查流程，減少 Bug 率 40%（導入 Git Flow 與 PR template 規範）","開發工具整合（MCP、Kiro-cli），開發效率提升 25%（自動化重複性任務與程式碼生成）","API 響應時間最佳化至平均 120ms（原 400ms）（透過資料庫索引、快取與分頁）"]}]',
      en: '[{"title":"Performance","items":["Supports 5,000+ concurrent WebSocket connections for real-time chat and interaction without lag (via Socket.IO room sharding and Redis relay)","Page load time optimized from 3.2s to 0.8s (75% performance improvement) (via route lazy-loading, tree-shaking, and resource preloading)","Broadcast console response latency maintained at < 100ms for better UX (via virtual scrolling and WebSocket state sync)","Infinite-scroll media management supporting 10,000+ media items with seamless loading (via Intersection Observer and prefetch paging)"]},{"title":"Quality","items":["Developed 32 reusable Vue 3 Composables, achieving 68% code reuse rate (abstracted shared logic with unified error handling)","Built 45 modular components supporting the live page, broadcast control, and member center (Atomic Design and props-driven architecture)","Unit test coverage reached 82% for higher stability (Vitest + Vue Test Utils covering core flows)","Implemented 12 MSW API Mock solutions to accelerate frontend-backend parallel development (standard API schemas shared across teams)"]},{"title":"Impact","items":["Integrated Payuni payment gateway, Keycloak SSO, and RBAC role-based access control (unified auth and authorization middleware)","Implemented 4 real-time notification mechanisms (WebSocket, SSE, subscription push, expiry alerts) (event-driven architecture with notification scheduler)","Supported 3 streaming modes: joint broadcast, co-stream, and OBS control (modular stream control and scene-switching logic)","Optimized the member center flow and increased subscription conversion by 35% (simplified checkout and A/B test validation)"]},{"title":"Efficiency","items":["Jenkins CI/CD automation with 8-12 automated deployments per week (Pipeline as Code with automated test gates)","Git branch management and code review process reduced bugs by 40% (Git Flow and PR template conventions)","Integrated MCP and Kiro-cli to improve development efficiency by 25% (automated repetitive tasks and code generation)","Optimized API response time to an average of 120ms from 400ms (database indexing, caching, and pagination)"]}]',
    },
  },
  {
    ...
    contribution: {
      zh: '[{"title":"性能提升","items":["3D CT 影像渲染平均載入時間 < 2s（透過 vtk.js 分塊載入與 Web Worker 非同步處理）","PDF 壓縮後檔案體積減少 60%（採用客戶端壓縮與適配性圖片壓縮策略）","學習平台影片首次緩衝時間 < 1.5s（透過 Vimeo CDN 與預載機制）","API 平均響應時間從 800ms 降至 200ms（透過查詢優化與 Redis 快取）"]},{"title":"程式品質","items":["建構 8 個可復用醫療圖表與表單元件（使用 Vue 3 + Composition API 抽象共用邏輯）","單元測試覆蓋率達 75%（以 Vitest 針對核心業務流程與 utility 編寫測試）","統一 API 錯誤處理與型別定義（建立 Zod / TypeScript 共享 schema）","資料庫 Schema 正規化減少冗餘（透過 ER 設計與 migration 管理）"]},{"title":"商業影響","items":["整合 OAuth 與 IP 白名單保護課程內容（透過 Vimeo 私有影片與閘道層驗證）","報價／帳款流程自動化縮短 50% 處理時間（透過狀態機與自動郵件通知）","醫療病歷管理流程數位化（取代紙本流程，減少資料查找時間）","多雲部署降低停機風險（Zeabur + Cloudways 雙平台備援機制）"]},{"title":"開發效率","items":["從 0 到 1 交付 3 個全端專案（採用模組化架構與可復用模板）","AI 輔助開發減少重複性程式碼撰寫 30%（使用 Cursor / Copilot 生成 boilerplate 與測試）","CI/CD 自動化部署減少手動上版時間 70%（透過 GitHub Actions 與 Zeabur webhooks）","統一前端元件庫縮短新頁面開發時間 40%（基於 Tailwind + Headless UI 建立設計系統）"]}]',
      en: '[{"title":"Performance","items":["3D CT scan rendering loads in < 2s on average (via vtk.js chunked loading and Web Worker async processing)","PDF compression reduces file size by 60% (via client-side compression and adaptive image downsampling)","Learning-platform videos buffer in < 1.5s on first play (via Vimeo CDN and preloading)","Average API response time reduced from 800ms to 200ms (via query optimization and Redis caching)"]},{"title":"Quality","items":["Built 8 reusable medical chart and form components (using Vue 3 + Composition API to abstract shared logic)","Unit-test coverage reached 75% (Vitest for core business flows and utilities)","Unified API error handling and type definitions (shared Zod / TypeScript schemas)","Normalized database schemas to reduce redundancy (ER design and migration management)"]},{"title":"Impact","items":["Integrated OAuth and IP whitelisting to protect course content (Vimeo private videos plus gateway validation)","Automated quote/billing workflow cut processing time by 50% (state-machine-driven with automated email notifications)","Digitized patient record management (replacing paper-based processes and reducing lookup time)","Multi-cloud deployment reduced downtime risk (Zeabur + Cloudways dual-platform backup)"]},{"title":"Efficiency","items":["Delivered 3 full-stack projects from 0 to 1 (modular architecture and reusable templates)","AI-assisted development reduced repetitive coding by 30% (Cursor / Copilot for boilerplate and tests)","CI/CD automation cut manual deployment time by 70% (GitHub Actions and Zeabur webhooks)","Unified frontend component library shortened new-page development by 40% (Tailwind + Headless UI design system)"]}]',
    },
  },
  {
    id: 'sun-life',
